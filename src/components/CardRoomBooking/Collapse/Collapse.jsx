...
function Collapse({ isOpen, rooms, onOpenModal }) {
    ...
    useEffect(() => {
        if (rooms && rooms.length > 0) {
            // Khởi tạo roomData với số phòng mặc định là 0
    ...
    const handleRoomCountChange = (index, value) => {
        const newRoomCount = parseInt(value);
        const newRoomData = [...roomData];

        newRoomData[index].count = newRoomCount;

        if (newRoomCount < newRoomData[index].adults) {
            newRoomData[index].adults = newRoomCount; // Giới hạn số người lớn không vượt quá số phòng
        }

        // Đặt số trẻ em và em bé về 0 khi số phòng là 0
        if (newRoomCount === 0) {
            newRoomData[index].adults = 0;
            newRoomData[index].children = 0;
            newRoomData[index].infants = 0;
        }
            ...
            {rooms && rooms.length > 0 ? (
                rooms.map((room, roomIndex) => (
                    <div key={room.id} className="w-full mb-6">
                        <div className="grid grid-cols-7 gap-5 items-center">
                            ...
                            <div className="col-span-1 font-inter">
                                <select
                                    ...
                                    value={roomData[roomIndex]?.count || 0} // Mặc định là 0
                        ...
                        {/* Phần chọn số người cho từng phòng, chỉ hiển thị nếu số phòng > 0 */}
                        {roomData[roomIndex]?.count > 0 && Array.from({ length: roomData[roomIndex]?.count }).map((_, index) => (
                            <div key={index}>
                                <div className="w-full grid grid-cols-12 gap-20 items-start mt-6 py-6 hover:bg-[#f5f5f5] rounded">
                                    ...
                                    <div className="col-span-3">
                                        <label htmlFor={`adult-${roomIndex}-${index}`} className="font-inter text-sm">Người lớn</label>
                                        <select
                                            ...
                                            id={`adult-${roomIndex}-${index}`}
                                            value={roomData[roomIndex]?.adults || 0} // Mặc định là 0
                                            onChange={(e) => handleChange(roomIndex, 'adults', e.target.value)}
                                        >
                                    ...
                                    <div className="col-span-3">
                                        <label htmlFor={`children-${roomIndex}-${index}`} className="font-inter text-sm">Trẻ em(6-11 tuổi)</label>
                                        <select
                                            className="p-1 mt-2 w-full border-2 border-gray-300 rounded bg-white relative focus:outline-none focus:ring focus:ring-yellow-500"
                                            id={`children-${roomIndex}-${index}`}
                                            value={roomData[roomIndex]?.children || 0} // Mặc định là 0
                                            onChange={(e) => handleChange(roomIndex, 'children', e.target.value)}
                                        >
                                            {[...Array(4)].map((_, i) => (
                                    ...
                                    <div className="col-span-3">
                                        <label htmlFor={`infant-${roomIndex}-${index}`} className="font-inter text-sm">Em bé(0-5 tuổi)</label>
                                        <select
                                            className="p-1 mt-2 w-full border-2 border-gray-300 rounded bg-white relative focus:outline-none focus:ring focus:ring-yellow-500"
                                            id={`infant-${roomIndex}-${index}`}
                                            value={roomData[roomIndex]?.infants || 0} // Mặc định là 0
                                            onChange={(e) => handleChange(roomIndex, 'infants', e.target.value)}
                                        >
                                            {[...Array(4)].map((_, i) => (
                                                <option key={i} value={i}>{i}</option>
                                            ))}
